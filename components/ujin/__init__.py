...
CONF_ON_SETTINGS_CHANGED = "on_settings_changed"
...
CONFIG_SCHEMA = cv.Schema({
    ...
    cv.Optional(CONF_ON_SETTINGS_CHANGED): cv.automation({
        cv.GenerateID(CONF_UJIN_ID): cv.use_id(UjinComponent),
    }),
...
async def to_code(config):
    ...
    for conf in config.get(CONF_ON_SETTINGS_CHANGED, []):
        await cg.register_automation(conf, (
            (cg.RawExpression(f"(({var})->get_settings_callback().add"), 
             cg.RawExpression(")"),
             cg.RawExpression("void(std::shared_ptr<esphome::ujin::DeviceState>)"))
        ))
