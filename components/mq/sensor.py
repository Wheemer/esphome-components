...
CONF_RL = "rl"
CONF_R0 = "r0"
CONF_R0_RESISTANCE = "r0_resistance"
CONF_CLEAN_AIR_RATIO = "clean_air_ratio"
CONF_VR = "vr"
...
CONFIG_SCHEMA = cv.All(
    cv.Schema({
        ...
        cv.Optional(CONF_CLEAN_AIR_RATIO): cv.positive_float,
        cv.Optional(CONF_R0_RESISTANCE): cv.positive_float,
...
async def to_code(config):
    ...
    if CONF_R0 in config:
        cg.add(var.set_clean_air_ratio(config[CONF_R0]))
    if CONF_CLEAN_AIR_RATIO in config:
        cg.add(var.set_clean_air_ratio(config[CONF_CLEAN_AIR_RATIO]))
    if CONF_R0_RESISTANCE in config:
        cg.add(var.set_R0(config[CONF_R0_RESISTANCE]))
