...
void MQSensor::setup() {
  ...
  if (this->r0_ > 0.0) {
    ESP_LOGCONFIG(TAG, "  Using predefined measured R0: %.2f kOhm", this->r0_);
    this->calibrated_ = true;
  } else if (this->clean_air_ratio_ > 0.0) {
    ESP_LOGCONFIG(TAG, "  Using clean-air RS/R0 ratio: %.2f", this->clean_air_ratio_);
  } else {
    ESP_LOGCONFIG(TAG, "  Using default clean-air RS/R0 ratio: %.2f", this->model_parameters_.get_ratio_in_clean_air());
  }
}
...
void MQSensor::dump_config() {
  ...
  if (this->r0_ > 0.0) {
    ESP_LOGCONFIG(TAG, "  R0: %.2f kOhm (measured/predefined)", this->r0_);
  } else if (this->calibrated_) {
    ESP_LOGCONFIG(TAG, "  R0: %.2f kOhm (calibrated)", this->r0_);
  } else {
    ESP_LOGCONFIG(TAG, "  R0: Not calibrated yet");
  }
  ESP_LOGCONFIG(TAG, "  Clean-air RS/R0 ratio: %.2f", this->clean_air_ratio_ > 0.0 ? this->clean_air_ratio_ : this->model_parameters_.get_ratio_in_clean_air());
...
float MQSensor::calculate_r0() {
  ...
  float ratio_in_clean_air = this->clean_air_ratio_ > 0.0 ? this->clean_air_ratio_ : this->model_parameters_.get_ratio_in_clean_air();
